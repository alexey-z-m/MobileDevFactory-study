...
struct Spending {
    ...
    func cheakingWayToTravel (way: Transport) {
        ...
        case .car(let distance , let cost):
            spend.spendingOnMovement(plane: nil, car: cost)
            print("Поедем на машине \(distance) ,стоимость поездки составит \(cost)")
        case .plane(let distance , let cost):
            spend.spendingOnMovement(plane: cost, car: nil)
            print("Полетим на самолете \(distance) ,стоимость полета составит \(cost)")
        }
    }
    func carOrPlane (costOfTripFromEkbToNov:Int ,spendingOnPlane: Int) -> () {
        if Int(costOfTripFromEkbToNov) < Int(spendingOnPlane) {
            spendingOnCar += [costOfTripFromEkbToNov]
            return print("я поеду на машине так как стоимость переезда на машине меньше стоимости билета самолета")
        } else {
            return print("я полечу на самолете так как стоимость полета меньше ")
        }
    }
}
...
// Новосибирск
var cheakWay = Spending()
cheakWay.carOrPlane(costOfTripFromEkbToNov: costOfTripFromEkbToNov, spendingOnPlane: 11000)
printStory {
...
cheakWay.cheakingWayToTravel(way: wayToTravel)
...
spend.spendingOnUsualDay(eating: 1200, accomodation: nil, other: nil)
...
spend.spendingOnUsualDay(eating: 20000, accomodation: hotelInRome.resultCost(), other: 12000)
...
spend.spendingOnUsualDay(eating: 17500, accomodation: hotelInMilan.resultCost(), other: 100000)

// В флоренцию
var florence = City(nameOfCity: "Флоренция", population: 1200000, distance: 390, fromToCity: "Milan")
florence.fromToCity = "Florence"
var spendOnBmw = Double(florence.distance) / 100 * rentCar.fuelСonsumption * costDiasel
...
spend.spendingOnUsualDay(eating: 13000, accomodation: nil , other: nil)

// мазара
var mazara = City(nameOfCity: "Mazara", population: 40000, distance: 1310, citiesCount: 6)
spendOnBmw = (Double(mazara.distance) / 100 * rentCar.fuelСonsumption) * costDiasel
...
spend.spendingOnUsualDay(eating: 50000, accomodation: mazaraHotel.resultCost(), other: 20000)
